...
(defclass 2dd-rect (2dd-editable-drawing 2dd-with-label)
  ()
...
(cl-defmethod 2dd-build-idx-edited ((rect 2dd-rect) (edit-idx integer) (move-vector 2dg-point) (viewport 2dd-viewport))
  ...
  (let ((pts (2dg-bounding-pts (oref rect _geometry)))
        (horizontal-pts 'nil)
        (vertical-pts 'nil))
...
(cl-defmethod 2dd-get-inner-canvas ((rect 2dd-rect))
  ...
  (with-slots (x-min y-min x-max y-max) (oref rect _geometry)
    (2dd-inner-canvas :x-min (+ x-min 2.0)
                      :y-min (+ y-min 2.0)
                      :x-max (- x-max 2.0)
                      :y-max (- y-max 3.0)
                      :parent rect)))
(cl-defmethod 2dd-set-from ((drawing-rect 2dd-rect) (source-rect 2dg-rect))
  ...
  (let ((geometry (oref drawing-rect _geometry)))
    (oset geometry x-min (oref source-rect x-min))
    (oset geometry x-max (oref source-rect x-max))
    (oset geometry y-min (oref source-rect y-min))
    (oset geometry y-max (oref source-rect y-max))))
